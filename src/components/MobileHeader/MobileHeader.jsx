...
class MobileHeader extends Component {
  ...
  collapseBodyAbout = () => (
    <div>
      {ABOUT_LINK_LIST.map(({ label, link }) => (
        <Link onClick={this.handleHamburgerClick} to={link}>
          {label}
        </Link>
      ))}
    ...
    <Container id="sideMenuContainer" className="py-4">
      <Row>
        ...
        <Col lg="12" className="my-5">
          <CollapseContainer
            trigger={this.collapseTriggerElementAbout}
            body={this.collapseBodyAbout}
          />
          <CollapseContainer
          ...
          {!this.props.isLoggedIn && (
            <Link onClick={this.handleHamburgerClick} to="/login">
              Login
            </Link>
          )}
          <Link
          ...
          {this.props.isLoggedIn && (
            <div>
              <Link onClick={this.handleHamburgerClick} to="/myaccount">
                My Account
              </Link>
              <Link
                onClick={() => {
  ...
  render() {
    return (
      <MobileHeaderContainer>
        <Container className="px-0">
          <Row>
            <Col lg="12 d-flex justify-content-between align-items-center">
              <Link id="mobileHeaderLogoLink" to="/">
                <img className="img-fluid" src={logo} />
              </Link>
              <div className="d-flex align-items-center">
                ...
                <button
                  ...
                  className={
                    ...
                    (this.state.sideMenuOpen ? "is-active" : "")
                  }
                  type="button"
                >
        ...
        >
          <SideMenuContainer>{this.sideMenuList()}</SideMenuContainer>
        </SwipeableDrawer>
      </MobileHeaderContainer>
    );
  }
